...
def get_all_pages():
    ...
    # print(src)
    soup = BS(src, "lxml")
    pages_count = soup.find("div", class_="bx-pagination-container")
    print(pages_count)
    # pages_count = int(soup.find("div", class_="bx-pagination-container").find_all("a")[-2].text)

    for i in range(1, pages_count + 1):
        url = f"https://shop.casio.ru/catalog/g-shock/filter/" \
              f"gender-is-male/apply/?PAGEN_1={i}"
...
def collect_data(pages_count):
    ...
    for page in range(1, pages_count):
        ...
        for item in items_cards:
            product_article = item.find("p",
                                        class_="product-item__articul").text.strip()
            product_price = item.find("p",
                                      class_="product-item__price").text.lstrip(
                "руб. ")
