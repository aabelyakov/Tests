...
def get_data(url):
    ...
    for item in range(1, 24):
        ...
        for article in articles:
            # project_url = "http://www.edutainme.ru" + article.find("div",
            #                                                        class_="txtBlock").find(
            #     "a").get("href")
        ...
        for project_url in projects_urls:
            ...
            try:
                project_name = project_data.find("div", class_="txt").find(
                    "h1").text
            except Exception:
                project_name = "No project name"

            try:
                project_short_description = project_data.find("div",
                                                              class_="txt").find(
                    "h4", class_="head").text
            except Exception:
            ...
            try:
                project_website = project_data.find("div",
                                                    class_="txt").find(
                    "p").find("a").text
            except Exception:
            ...
            try:
                project_full_description = project_data.find("div",
                                                             class_="textWrap").find(
                    "div", class_="rBlock").text
            except Exception:
            ...
            project_full_description = replace_string(
