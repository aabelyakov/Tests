...
count = 0
print(2, f"Всего итераций: {iteration_count}")
...
for category_name, category_href in all_categories.items():
    ...
    table_head = soup.find(class_="mzr-tc-group-table").find("tr").find_all(
        "th")
    ...
    with open(f"data/{count}_{category_name}.csv", "w",
    ...
    products_data = soup.find(class_="mzr-tc-group-table").find(
        "tbody").find_all("tr")
    ...
    count += 1
    print(4, f"# Итерация {count}. {category_name} записан...")
    iteration_count = iteration_count - 1

    if iteration_count == 0:
        print(5, "Работа завершена")
        break

    print(6, f"Осталось итераций: {iteration_count}")
