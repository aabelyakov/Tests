...
def get_data():
    ...
    url = "https://roscarservis.ru/catalog/legkovye/" \
          "?form_id=catalog_filter_form&filter_mode=params&sort=" \
          "asc&filter_type=tires&arCatalogFilter_458_1500340406=" \
          "Y&set_filter=Y&arCatalogFilter_463=668736523&PAGEN_1=1"
    ...
    for page in range(1, pages_count + 1):
        url = f"https://roscarservis.ru/catalog/legkovye/" \
              f"?form_id=catalog_filter_form&filter_mode=params&sort=" \
              f"asc&filter_type=tires&arCatalogFilter_458_1500340406=" \
              f"Y&set_filter=Y&arCatalogFilter_463=668736523&PAGEN_1={page}"

        r = requests.get(url=url, headers=headers)
        data = r.json()
        items = data["items"]

        possible_stores = ["discountStores", "fortochkiStores",
                           "commonStores"]
        for item in items:
            total_amount = 0
