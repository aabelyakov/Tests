...
def get_data():
    ...
    for page in range(1, pages_count + 1):
        ...
        url = f"https://www.labirint.ru/genres/2308/?available=" \
              f"1&paperbooks=1&display=table&page={page}"

        response = requests.get(url=url, headers=headers)
        soup = BS(response.text, "lxml")

        books_items = soup.find("tbody",
                                class_="products-table__body").find_all("tr")
        ...
        for bi in books_items:
            ...
            try:
                ...
                book_publishing = ":".join(
                    [bp.text for bp in book_publishing])
            except:
            ...
            try:
                book_new_price = int(
                    book_data[3].find("div", class_="price").find(
            ...
            try:
                book_old_price = int(book_data[3].find("span",
                                                       class_="price-gray").text.strip().replace(
                    " ", ""))
            except:
...
def main():
    ...
    print(f"Время, затраченное на работу скрипта: {delta_time}")
